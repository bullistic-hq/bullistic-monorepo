...
const BULLISTIC_WEBSITE_TITLE = "Bullistic - NFT Marketplace";
const BULLISTIC_DESCRIPTION =
  "Bullistic is the digital art marketplace for Solana. Create and collect amazing digital art, without harming the environment.";
const ONE_DAY_IN_SECONDS = 3600 * 24;

const DEFAULT_TAG_VALUES = {
  ASSET_HEIGHT: "500",
  ASSET_WIDTH: "500",
  OG_DESCRIPTION: BULLISTIC_DESCRIPTION,
  OG_IMAGE:
    "https://firebasestorage.googleapis.com/v0/b/bullistic-ed6b4.appspot.com/o/bullistic-facebook-preview2.png?alt=media&token=fc494364-5372-4a3d-a947-5f7810abd66d",
  OG_TITLE: BULLISTIC_WEBSITE_TITLE,
  OG_TYPE: "website",
  OG_URL: "https://www.bullistic.xyz/",
  TWITTER_CARD: "summary_large_image",
  TWITTER_DESCRIPTION: BULLISTIC_DESCRIPTION,
  TWITTER_IMAGE:
    "https://firebasestorage.googleapis.com/v0/b/bullistic-ed6b4.appspot.com/o/bullistic-twitter-preview2.png?alt=media&token=191f304a-5ca4-490a-98b8-cbab9a896088",
  TWITTER_SITE: "@bullistic",
  TWITTER_TITLE: BULLISTIC_WEBSITE_TITLE,
};
...
function getApiUrl(url: string, apiUrlFromEnv: string) {
  ...
  // against dev
  if (url.includes("dev.bullistic.xyz")) {
    return "https://apidev.bullistic.xyz";
  }
...
class HeadHandler {
  ...
  getTitle() {
    ...
    if (this.pageType === PageType.CampaignPage && name) {
      return `${name} | ${BULLISTIC_WEBSITE_TITLE}`;
    }
    ...
    if (this.pageType === PageType.UserProfilePage) {
      if (name && username) {
        return `${name} (@${username}) | ${BULLISTIC_WEBSITE_TITLE}`;
      }
      if (name && !username) {
        return `${name} | ${BULLISTIC_WEBSITE_TITLE}`;
      }
      if (username && !name) {
        return `@${username} | ${BULLISTIC_WEBSITE_TITLE}`;
      }
    }

    return BULLISTIC_WEBSITE_TITLE;
  }
  ...
  getOgUrl() {
    ...
    if (this.pageType === PageType.NftListingPage && mint) {
      ...
      return `https://bullistic.xyz/@${username ?? ""}/${mint}`;
    }

    if (this.pageType === PageType.CampaignPage && username && campaignSlug) {
      return `https://bullistic.xyz/@${username}/campaigns/${campaignSlug}`;
    }

    if (this.pageType === PageType.UserSeriesPage && username && seriesSlug) {
      return `https://bullistic.xyz/@${username}/series/${seriesSlug}`;
    }

    if (this.pageType === PageType.UserProfilePage && username) {
      return `https://bullistic.xyz/@${username}`;
    }
